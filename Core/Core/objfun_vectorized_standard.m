...
function [vector_objvals, log] = objfun_vectorized_standard(model, mat_vector_config, stateparfromtoreduced, timeout, errtype, variability, var_obj_prctile, virtual_pop_X0, virtual_pop_par, X_ref_var, log)
...
if variability
    for nconfig = 1:Nconfigs
        ...
        if any(isnan(errout)) || any(isnan(errdyn100))
            vector_objvals(nconfig, :) = [model.I.nstates+1, 3e6, 3e6, 3e6];
