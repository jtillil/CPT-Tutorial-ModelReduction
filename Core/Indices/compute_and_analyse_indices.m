...
function model = compute_and_analyse_indices(model,action)
...
switch action
    case 'compute'
        saveresults = 0;      
        [model.ir, model.contr, model.obs] = compute_ir_indices(model,saveresults);

        % compute env, pneg, cneg, pss (and int) indices
