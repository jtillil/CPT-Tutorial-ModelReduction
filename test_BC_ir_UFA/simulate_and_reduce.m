function model = simulate_and_reduce(scenario,varargin)
...
% seed: seed for random parameters (default: 0)
...
% preparation    
addpath('blood_coagulation_modelspecification/','ir_index_modelreduction/','blood_coagulation_modelspecification/optional/')
...
par(I.v43) = 0; % to avoid numerical problem
model.par = par;
...
switch model.scenario
    case 'in_vivo_warfarin'
        ...
        I.h = @(~,x,varargin) arrayfun(@(n) (simulate_and_reduce('in_vitro_warfarin', 'initial_values', x(n,:),'parameters',varargin{2}.par).Y_ref(1)), 1:size(x,1))'/3.068837*1000;%simredmodelODE(x);%simredmodelODE(x);
    case 'in_vivo_ss'
        if sum(output_time)==0
            tspan = [0 24*30]; % simulation time span (30 days)
        else
...
if sample_size||(any(any(factors_par)) && any(any(factors_init))) || (any(any(X0s)) && any(any(pars)))
    if (any(any(X0s)) && any(any(pars)))
        ...
        if sample_size
            ...
            if sample_size>=1000 || force_lhs
                factors_par=exp(lhsnorm(zeros(I.npar,1),sigma*eye(I.npar),sample_size));
                factors_init=exp(lhsnorm(zeros(I.nstates,1),sigma*eye(I.nstates),sample_size));
            ...
            % choose genotypes deterministically according to allele frequencies
    ...
    elseif ~isempty(ref_sol)
        ...
        elseif ~isempty(ref_dyn)
            ...
            fprintf("reduced model topology is needed via 'I_reduced' or 'ref_dyn',... to simulate only_reduced")
        end
        return
    end
    model.Y_refs= Y_refs;
...
% error definition
if strcmp(model.scenario,'in_vivo_warfarin') || strcmp(model.scenario,'in_vitro_PTtest_highTF')
...
if reduce
    switch model.scenario
        case 'in_vivo_warfarin'
            ir_file="results/ir_in_vivo_warfarin.mat";
        case 'in_vitro_warfarin'
            ir_file="results/ir_in_vitro_warfarin.mat";
        otherwise
            fprintf('\n--> unknown model.scenario :-( Please fix!\n\n');
            error('')
    end
    load_ir_indices=ir;
    if ~load_ir_indices
        if strcmp(model.scenario, 'in_vitro_warfarin') 
    ...
    if isfield(model,'pars')
        redmodel=model_order_reduction_robust(model,seqofstates,model.relerrTOL,'share',share);
    else
