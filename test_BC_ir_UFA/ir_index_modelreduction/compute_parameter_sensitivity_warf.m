...
function [obs]  =  compute_parameter_sensitivity_warf(model)
...
for k = 1:I.nstates
    obs(k)   = sqrt( 1/t_ref(end) * trapz(t_tstar, (0.194*Y_ref(1)*(Jac_y(I.II,k,:)/X_ref(1,I.II)+Jac_y(I.VII,k,:)/X_ref(1,I.VII)+Jac_y(I.X,k,:)/X_ref(1,I.X))).^2) );
end
