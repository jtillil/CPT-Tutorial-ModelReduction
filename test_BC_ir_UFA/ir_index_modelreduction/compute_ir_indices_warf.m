...
function [ir,contr,obs]  =  compute_ir_indices_warf(model)
...
for ts = 1:ntstar-1
    ...
    for k = 1:I.nstates
        obs(ts,k)   = sqrt( 1/t_ref(end) * trapz(t_tstar, (0.194*Y_ref(ts)*(Jac_y(I.II,k,:)/X_ref(ts,I.II)+Jac_y(I.VII,k,:)/X_ref(ts,I.VII)+Jac_y(I.X,k,:)/X_ref(ts,I.X))).^2) );
...
% initialise output
extodejacpat = sparse(nstates+nstates^2,nstates+nstates^2);
